...
def compare_models(prompt: str) -> dict:
    ...
    cost = (len(gpt4o_text.split()) / 0.75) / 1000 \
           * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    cost = (len(mini_text.split()) / 0.75) / 1000 \
...
def run_assistant(
    ...
    total_cost = 0.0
    while True:
        # 1. Kiểm tra điều kiện dừng số lượt tối đa
        if max_turns is not None and num_turns >= max_turns:
            break
        # 2. Đọc tin nhắn và kiểm tra lệnh thoát
        user_msg = get_input()
        if user_msg.strip().lower() in ("quit", "exit"):
            break
        messages = [{"role": "system", "content": persona}] + history + [
        ...
        for chunk in stream:
            ...
            assistant_msg += delta
        print()
        # 5. Cập nhật history (giữ tối đa 3 lượt cuối = 6 messages)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": assistant_msg})
        history = history[-6:]
        # 6. Cộng dồn thống kê token và chi phí
        cost_info = estimate_cost(user_msg, assistant_msg, OPENAI_MODEL)
